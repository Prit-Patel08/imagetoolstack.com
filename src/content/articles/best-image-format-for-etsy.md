...
```mermaid
graph TD
    A[Export Product Photography] --> B{Choose Image Format}
    B -- JPEG .jpg at 85% Quality --> C[File Size: 450KB to 800KB]
    C --> D[Fast Mobile App Load Times]
    D --> E[Triggers Full-Screen HD Zoom]
    B -- Uncompressed PNG --> F[File Size: 3MB to 6MB]
    F --> G[Slow Mobile Load -> Bounces Buyers]
...
```mermaid
graph LR
    A[Capture Photo at 2000x2500px 4:5 Ratio] --> B[Center Main Product Subject]
    B --> C[Etsy Mobile Search Thumbnail 4:5 Render]
    B --> D[Etsy Desktop Grid 1:1 Auto-Crop]
    C --> E[Full Product Visibility]
    D --> F[No Product Off-Screen Clipping]
```
